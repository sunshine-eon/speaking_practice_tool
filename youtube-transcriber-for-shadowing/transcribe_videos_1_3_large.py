"""Transcribe videos 1-3 using large Whisper model."""
...
# Video IDs for latest 5 Lenny videos (not yet transcribed with large model)
VIDEO_IDS = [
    "scsW6_2SPC4",  # Latest video 1: OpenAI's CPO
    "ImSvm11GR0Y",  # Latest video 2: A framework for PM skill development
    "EgBOVDzUUAA",  # Latest video 3: Kunal Shah on winning in India
    "h-KVGHoQ_98",  # Latest video 4: The nature of product
    "uDq6_CPaRjM",  # Latest video 5: Vision, conviction, and hype
]
...
def main():
    """Main function to transcribe videos 1-3 with large model."""
    ...
    print("="*60)
    print("Transcribing Latest 5 Lenny Videos with Large Whisper Model")
    print("="*60)
    print(f"Videos to transcribe: {len(VIDEO_IDS)}")
    ...
    for video_idx, video_id in enumerate(VIDEO_IDS, 1):
        ...
        # Ask if user wants to continue (except for last video)
        if video_idx < len(VIDEO_IDS) and "--auto" not in sys.argv and "-y" not in sys.argv:
