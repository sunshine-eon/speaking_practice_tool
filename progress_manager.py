...
def update_progress(progress: Dict[str, Any], activity_id: str, week_key: str = None, 
    ...
    Args:
        ...
        activity_id: One of 'voice_journaling', 'shadowing_practice', 'weekly_speaking_prompt'
        week_key: Week key (defaults to current week)
        completed: Whether the activity is completed
        day: For voice_journaling, shadowing_practice and weekly_speaking_prompt, the day to mark (format: 'YYYY-MM-DD')
    """
    if week_key is None:
    ...
    if activity_id in ["voice_journaling", "shadowing_practice", "weekly_speaking_prompt"]:
        if day is None:
